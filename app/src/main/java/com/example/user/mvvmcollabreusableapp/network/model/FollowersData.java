...
public class FollowersData implements Parcelable {

    public static final Creator<FollowersData> CREATOR = new Creator<FollowersData>() {
        @Override
        public FollowersData createFromParcel(Parcel source) {
            return new FollowersData(source);
        }

        @Override
        public FollowersData[] newArray(int size) {
            return new FollowersData[size];
        }
    };
    ...
    private List<FollowersData> replies;

    public FollowersData() {
    }

    protected FollowersData(Parcel in) {
        ...
        in.readList(this.followers, Followers.class.getClassLoader());
        this.selectedUsers = new ArrayList<>();
        in.readList(this.selectedUsers, String.class.getClassLoader());
        this.attachments = in.createStringArrayList();
        this.tags = new ArrayList<>();
        in.readList(this.tags, Tags.class.getClassLoader());
        this.replies = new ArrayList<>();
        in.readList(this.replies, FollowersData.class.getClassLoader());
    }
    ...
    public List<FollowersData> getReplies() {
        return replies;
    }

    public void setReplies(List<FollowersData> replies) {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeList(this.selectedUsers);
    ...
    public static class Followers implements Parcelable {
        ...
        protected Followers(Parcel in) {
            ...
            this.emailValidated = in.readByte() != 0;
            this.isAdmin = in.readByte() != 0;
            this.permissions = in.readParcelable(PermissionsBeanX.class.getClassLoader());
            this.id = in.readString();
            this.updatedAt = in.readString();
            this.createdAt = in.readString();
            this.email = in.readString();
        ...
        public static class PermissionsBeanX implements Parcelable {

            public static final Creator<PermissionsBeanX> CREATOR = new Creator<PermissionsBeanX>() {
                @Override
                public PermissionsBeanX createFromParcel(Parcel source) {
                    return new PermissionsBeanX(source);
                }

                @Override
                public PermissionsBeanX[] newArray(int size) {
                    return new PermissionsBeanX[size];
                }
            };
            @SerializedName("is_admin")
            private boolean isAdmin;
            ...
            public PermissionsBeanX() {
            }

            protected PermissionsBeanX(Parcel in) {
                this.isAdmin = in.readByte() != 0;
                this.canMakeAdmin = in.readByte() != 0;
            }
